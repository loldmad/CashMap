...
function nonNeg(v) { return Math.max(0, toNum(v)); }

function incomeEngine() {
  return {
    weeklyIncome: state.incomeStreams.reduce((s, i) => s + nonNeg(i.rate) * nonNeg(i.units), 0)
  };
}
...
function taxEngine(grossWeekly) {
  ...
  const deductionRows = state.tax.deductions.map((d) => {
    const amount = d.mode === 'percent' ? grossWeekly * (nonNeg(d.value) / 100) : nonNeg(d.value);
    return { ...d, amount };
  });
  const optional = deductionRows.reduce((s, d) => s + d.amount, 0);
  const takeHome = Math.max(0, grossWeekly - federalTax - stateTax - payrollTax - optional);
...
function smartGoalPlan(savedWeekly) {
  const goals = state.goals.map((g) => ({ ...g, amount: nonNeg(g.amount), current: nonNeg(g.current) }));
...
function moneyFlow() {
  ...
  const savedWeekly = Math.max(0, tax.takeHome * (nonNeg(state.savingsRate) / 100));
...
function renderIncome({ income }) { /* unchanged functional block */
...
function renderPaycheck({ income, tax, pay }) { /* unchanged */
  ...
  state.tax.deductions.forEach((d) => {
...
function renderGoals({ savedWeekly, groupGoals }) { /* unchanged */
  ...
  document.getElementById('saveRate').onchange = (e) => { state.savingsRate = nonNeg(e.target.value); render(); };
...
function renderCashflow({ savedWeekly }) {
  ...
  const canvas = document.getElementById('flowCanvas'); const ctx = canvas.getContext('2d'); const weeks = toNum(state.chartWeeks, 26);
  ...
  for (let wk = 0; wk <= weeks; wk++) { const x = left + (w / weeks) * wk; const total = savedWeekly * wk; const y = bottom - (Math.min(max, total) / max) * h; if (wk === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }
  ctx.stroke();
  document.getElementById('weeksSel').onchange = (e) => { state.chartWeeks = toNum(e.target.value, 26); render(); };
}

function onStreamEdit(e) { const { id, field } = e.target.dataset; const s = state.incomeStreams.find((x) => x.id === id); if (!s) return; s[field] = field === 'name' || field === 'type' ? e.target.value : nonNeg(e.target.value); render(); }
function bindTaxControls() {
  ...
  document.querySelectorAll('[data-ded-id]').forEach((el) => el.onchange = (e) => { const id = e.target.dataset.dedId; const field = e.target.dataset.field; const row = state.tax.deductions.find((d) => d.id === id); if (!row) return; row[field] = field === 'name' || field === 'mode' ? e.target.value : nonNeg(e.target.value); render(); });
...
function onGoalEdit(e) { const goalId = e.target.dataset.goalId; const field = e.target.dataset.field; const goal = state.goals.find((g) => g.id === goalId); if (!goal) return; goal[field] = field === 'name' ? e.target.value : nonNeg(e.target.value); render(); }
...
function migrateState(raw) {
  const next = { ...defaultState, ...raw, tax: { ...defaultState.tax, ...(raw.tax || {}) } };
  if (!Array.isArray(next.tax.deductions) || !next.tax.deductions.length) next.tax.deductions = recommendedDeductions.map((d) => ({ id: crypto.randomUUID(), ...d }));
  next.tax.deductions = next.tax.deductions.map((d) => ({ id: d.id || crypto.randomUUID(), name: d.name || 'Deduction', mode: d.mode === 'percent' ? 'percent' : 'fixed', value: nonNeg(d.value) }));
  if (!Array.isArray(next.goals)) next.goals = defaultState.goals;
  next.goals = next.goals.map((g) => ({ id: g.id || crypto.randomUUID(), name: g.name || 'Goal', amount: nonNeg(g.amount), current: nonNeg(g.current) }));
  if (!Array.isArray(next.incomeStreams) || !next.incomeStreams.length) next.incomeStreams = defaultState.incomeStreams;
  return next;
}
...
document.addEventListener('keydown', (e) => {
  if (!document.activeElement?.classList?.contains('tab-btn')) return;
  const current = [...tabs].indexOf(document.activeElement);
  if (current < 0) return;
  if (e.key === 'ArrowRight') { e.preventDefault(); tabs[(current + 1) % tabs.length].focus(); }
  if (e.key === 'ArrowLeft') { e.preventDefault(); tabs[(current - 1 + tabs.length) % tabs.length].focus(); }
  if (e.key === 'Home') { e.preventDefault(); tabs[0].focus(); }
  if (e.key === 'End') { e.preventDefault(); tabs[tabs.length - 1].focus(); }
  if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setTab(document.activeElement.dataset.tab); render(); }
});
